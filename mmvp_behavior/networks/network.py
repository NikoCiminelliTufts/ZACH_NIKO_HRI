...
class network(nn.Module):
    ...
    def build_modalities_block(self):
        ...
        if self.VIBRO_LAYER != 0:
            ...
            self.vibro_f.RELUhead = vibro_head(self.lstm_size[3])
    ...
    def forward(self, images, haptics, audios, behaviors, vibros, train=True):
        ...
        if self.k == -1 or not train: # it won't enter this during training
            feedself = True
            # print("feedself_if", feedself)
        else:
            num_ground_truth = round(images[0].shape[0] * (self.k / (math.exp(self.iter_num/self.k) + self.k)))
            feedself = False
            self.iter_num += 1
            # print("num_ground_truth", num_ground_truth)
            # print("feedself_else", feedself)
