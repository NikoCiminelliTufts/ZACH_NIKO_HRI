...
class PushDataset(Dataset):
    def __init__(self, root, image_transform=None, action_transform=None, state_transform=None, loader=npy_loader, device='cpu'):
        if not os.path.exists(root):
            raise FileExistsError('{0} does not exist!'.format(root))
...
class CY101Dataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # print('vision.shape before', vision.shape)
        # print('haptic.shape before', haptic.shape)
        # print('audio.shape before', audio.shape)
        # print('vibro.shape before', vibro.shape)
        ...
        # print("vision.shape after", vision.shape)
        # print("haptic.shape after", haptic.shape)
        # print("audio.shape after", audio.shape)
        # print("vibro.shape after", vibro.shape)
    ...
    class Standardizer:
        def __init__(self, mean, std):
            if not isinstance(mean, torch.Tensor) and not isinstance(std, torch.Tensor):
                raise TypeError("Expected mean and std to be  torch.Tensor")
            self.mean = mean
            self.std = std
